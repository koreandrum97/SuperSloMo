...
parser.add_argument("--validation_batch_size", type=int,
                    default=10, help='batch size for validation. Default: 10.')
...
parser.add_argument("--milestones", type=list, default=[
                    100, 150], help='Set to epoch values where you want to decrease learning rate by a factor of 0.1. Default: [100, 150]')
...
flowComp.to(device)
flowComp = torch.nn.DataParallel(flowComp)

ArbTimeFlowIntrp = model.UNet(28, 7)
ArbTimeFlowIntrp.to(device)
ArbTimeFlowIntrp = torch.nn.DataParallel(ArbTimeFlowIntrp)
...
mean = [0.429, 0.431, 0.397, 0.429, 0.431]
std = [1, 1, 1, 1, 1]
...
vgg16_conv_4_3[0] = nn.Conv2d(5, 64, kernel_size=(
...
def validate():
    ...
    with torch.no_grad():
        for validationIndex, (validationData, validationFrameIndex) in enumerate(validationloader, 0):
            ...
            # For tensorboard
            if (flag):
                retImg = torchvision.utils.make_grid([revNormalize(frame0[0]), revNormalize(
                    frameT[0]), revNormalize(Ft_p.cpu()[0]), revNormalize(frame1[0])], padding=10)
                flag = 0
...
for epoch in range(dict1['epoch'] + 1, args.epochs):
    ...
    for trainIndex, (trainData, trainFrameIndex) in enumerate(trainloader, 0):
        ...
        if ((trainIndex % args.progress_iter) == args.progress_iter - 1):
            ...
            print(" Loss: %0.6f  Iterations: %4d/%4d  TrainExecTime: %0.1f  ValLoss:%0.6f  ValPSNR: %0.4f  ValEvalTime: %0.2f LearningRate: %f" %
                  (iLoss / args.progress_iter, trainIndex, len(trainloader), end - start, vLoss, psnr, endVal - end, get_lr(optimizer)))
    ...
    if ((epoch % args.checkpoint_epoch) == args.checkpoint_epoch - 1):
        ...
        torch.save(dict1, args.checkpoint_dir + "/SuperSloMo" +
